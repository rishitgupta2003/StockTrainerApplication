...
@app.route('/home/buyList/adani-ent' , methods = ['GET' , 'POST'])
def buy():
    msg =''
    if session['loggedin'] == False :
        msg = 'login first'
    ...
    if request.method == 'POST':
        ...
        price = currentPrice('ADANIENT.NS') 
        cost = price * (int)(shares)
        if((float)(balance[0]) >= (float)(cost)):  
            new_balance = (float)(balance[0]) - (float)(cost)
            msg = buyStock(session['id'] , 'ADANIENT.NS' , price , shares , cost , new_balance)
        else:
            msg = "NOT ENOUGH BALANCE"
                     
    return render_template('BUYadani-ent.html', plot=plot_data('ADANIENT.NS') , msg = msg , price = (str)(currentPrice('ADANIENT.NS')))


@app.route('/home/sellList/adani-ent' , methods = ['GET' , 'POST'])
def sell():
    msg =''
    if session['loggedin'] == False :
        msg = 'login first'
    ...
    if request.method == 'POST':
        ...
        if(sa >= (int)(shares)):
            price = currentPrice('ADANIENT.NS')
            msg = sellStock(userid , price , shares , sa , 'ADANIENT.NS')
        else:
            msg = "Not Enough Stocks"
            

        
    return render_template('sell-adani.html' , msg = msg , plot = plot_data('ADANIENT.NS') , price = currentPrice('ADANIENT.NS'))
